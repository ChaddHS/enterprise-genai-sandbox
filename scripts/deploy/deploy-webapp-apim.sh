...
eval REACT_APP_BACKEND_WEBSOCKET=$(echo $DEPLOYMENT_JSON | jq -r '.properties.outputs.webapiUrl.value')
echo "REACT_APP_BACKEND_WEBSOCKET: $REACT_APP_BACKEND_WEBSOCKET"
# eval WEB_API_NAME=$(echo $DEPLOYMENT_JSON | jq -r '.properties.outputs.webapiName.value')
# echo "WEB_API_NAME: $WEB_API_NAME"
...
echo "REACT_APP_BACKEND_WEBSOCKET=https://$WEB_API_URL/" >> $ENV_FILE_PATH
...
# added staticwebapp
#yes | cp -r ./staticwebapp.config.json ./build/staticwebapp.config.json
...
#az staticwebapp enterprise-edge show --name swa-copichat-fofunnuufhjhk --resource-group rg-enterprise-genai-playground
